...
class TestFitOneTemperatureDependence(object):
    def make_score_fcn(self, fold_model_factory, unfold_model_factory,
                       ...
                       unfold_data_predictor, target_data):
    ...
    def predicted_rate_similar_to_true_rate(self):
        ...
        '''
        target_data = TemperatureDependenceTargetData()
        target_data.load_data('Pin_WW')
        features = target_data.get_feature()
        fold_beta_array = features['fold']
        unfold_beta_array = features['unfold']
        id_list = range(len(fold_beta_array))
        fold_model_factory = CoopCollectionFactory(id_list, 'beta',
                                                   fold_beta_array)
        id_list = range(len(unfold_beta_array))
        unfold_model_factory = CoopCollectionFactory(id_list, 'beta',
                                                     unfold_beta_array)
